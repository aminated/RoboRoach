...
public class RoboRoachManager {
    ...
    public static final UUID BYB_ACCEL_SERVICE = new UUID((0xFFA0L << 32) | 0x1000, GattUtils.leastSigBits);
    ...
    public static final UUID ROBOROACH_STIMULATE = new UUID((0xB2B5L << 32) | 0x1000, GattUtils.leastSigBits);
    ...
    public static final UUID ROBOROACH_ACCEL_ENABLE  = new UUID((0xFFA1L << 32) | 0x1000, GattUtils.leastSigBits);
    public static final UUID ROBOROACH_ACCEL_X  = new UUID((0xFFA3L << 32) | 0x1000, GattUtils.leastSigBits);
    public static final UUID ROBOROACH_ACCEL_Y  = new UUID((0xFFA4L << 32) | 0x1000, GattUtils.leastSigBits);
    public static final UUID ROBOROACH_ACCEL_Z  = new UUID((0xFFA5L << 32) | 0x1000, GattUtils.leastSigBits);
    ...
    private BluetoothGattService    mAccelService;
    ...
    private static int rrAccelX = 0;
    private static int rrAccelY = 0;
    private static int rrAccelZ = 0;
    private static int accelEnabled = 0;
    public void setAccel(Accelerometer accel) {
        this.accel = accel;
    }
    /* Call this function to clean up when something goes wrong (i.e. mBluetoothGatt == null) */
    public void connectionFail(){
        // Clean up state
        accelEnabled = 0;
        // Call regular disconnect routine.
        disconnect();

    }
    public boolean isAccelEnabled(){
        return accelEnabled == 1;
    }
    public void setAccelEnabled(boolean enabled){
        accelEnabled = enabled ? 1 : 0;
        boolean enable = accelEnabled == 1;
        if(mBluetoothGatt == null) {
            connectionFail();
            return;
        }
        if(mAccelService == null) mAccelService = mBluetoothGatt.getService(BYB_ACCEL_SERVICE);
        if(mAccelService != null && mBluetoothGatt != null) {

            final byte[] dataToWrite = new byte[]{(byte) accelEnabled};
            final BluetoothGattCharacteristic ch = mAccelService.getCharacteristic(ROBOROACH_ACCEL_ENABLE);
            ...
            setNotificationForCharacteristic(mAccelService.getCharacteristic(ROBOROACH_ACCEL_X), enable);
            SystemClock.sleep(100);

            setNotificationForCharacteristic(mAccelService.getCharacteristic(ROBOROACH_ACCEL_Y), enable);
            SystemClock.sleep(100);

            setNotificationForCharacteristic(mAccelService.getCharacteristic(ROBOROACH_ACCEL_Z), enable);

        }
    }
    public void toggleAccel(){
        setAccelEnabled(accelEnabled == 0);
    }
    public void requestAccelValues(){
        requestCharacteristicValue(mAccelService.getCharacteristic(ROBOROACH_ACCEL_X));
        requestCharacteristicValue(mAccelService.getCharacteristic(ROBOROACH_ACCEL_Y));
    }
    ...
    public int getAccelX() {
        final BluetoothGattCharacteristic ch = mAccelService.getCharacteristic(ROBOROACH_ACCEL_X);
        mBluetoothGatt.readCharacteristic(ch);
        rrAccelX = 50;
        return rrAccelX;
    }
    public int                        getAccelY() { return rrAccelY; }
    public int                        getAccelZ() { return rrAccelZ; }
    ...
    public void turnRight() {
        ...
        final byte[] dataToWrite = new byte[] { (byte)0x02 };
        final BluetoothGattCharacteristic ch = mRoboRoachService.getCharacteristic(ROBOROACH_STIMULATE);
    ...
    public void turnLeft() {
        ...
        // ... and then "commit" changes to the peripheral
        mBluetoothGatt.writeCharacteristic(ch);
    }// This is added code starts here
  /*  public void turnUp() {
        if (mBluetoothAdapter == null || mBluetoothGatt == null || mRoboRoachService == null) return;

        final byte[] dataToWrite = new byte[] { (byte)0x32 };
     //   final BluetoothGattCharacteristic ch = mRoboRoachService.getCharacteristic(ROBOROACH_CHAR_LED_UUID );
    ...
    public void turnDown() {
        ...
        final byte[] dataToWrite = new byte[] { (byte)0x16 };
     //   final BluetoothGattCharacteristic ch = mRoboRoachService.getCharacteristic(ROBOROACH_CHAR_LED_UUID );
...
// added code ends here
    public void stimBoth() {
        if (mBluetoothAdapter == null || mBluetoothGatt == null || mRoboRoachService == null) return;

        final byte[] dataToWrite = new byte[] { (byte)0x03 };
        final BluetoothGattCharacteristic ch = mRoboRoachService.getCharacteristic(ROBOROACH_STIMULATE);
    ...
    public void updateAccel(boolean status){
        if (mBluetoothAdapter == null || mBluetoothGatt == null || mRoboRoachService == null) return;

        final byte[] dataToWrite = new byte[] { 0x01 };
        final BluetoothGattCharacteristic ch = mAccelService.getCharacteristic(ROBOROACH_ACCEL_ENABLE);
        ch.setValue(dataToWrite);
        mBluetoothGatt.writeCharacteristic(ch);
    }
    ...
    public void getSupportedServices() {
        ...
        mAccelService = mBluetoothGatt.getService(BYB_ACCEL_SERVICE);
    ...
    public void getCharacteristicValue(BluetoothGattCharacteristic ch) {
        ...
        if (ch.getUuid().equals(ROBOROACH_ACCEL_X)) {
            rrAccelX = ch.getIntValue(BluetoothGattCharacteristic.FORMAT_UINT8, 0);
            accel.pushX(rrAccelX);
        }

        if (ch.getUuid().equals(ROBOROACH_ACCEL_Y)) {
            rrAccelY = ch.getIntValue(BluetoothGattCharacteristic.FORMAT_UINT8, 0);
            accel.pushZ(rrAccelY);

        }

        if (ch.getUuid().equals(ROBOROACH_ACCEL_Z)){
            rrAccelZ = ch.getIntValue(BluetoothGattCharacteristic.FORMAT_UINT8, 0);
            accel.pushY(rrAccelZ);
    ...
    private final BluetoothGattCallback mBleCallback = new BluetoothGattCallback() {
        ...
        public void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
            ...
            if(status == BluetoothGatt.GATT_SUCCESS) {
                if (characteristic.getUuid().equals(ROBOROACH_STIMULATE) ) {
